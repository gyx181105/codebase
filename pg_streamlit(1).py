...
def draw_chart(df_week_node, df_month_node, chart_name):
    ...
    mean_line1 = alt.Chart(df_week_node[['节点号', chart_name]]).mark_rule(color='green').encode(y=alt.value(mean1))
    # 7日文本标记
    ...
    mean_line2 = alt.Chart(df_month_node[['节点号', chart_name]]).mark_rule(color='green').encode(y=alt.value(mean2))
    # 30日文本标记
